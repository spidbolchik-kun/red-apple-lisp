        ...
        (if (symbol? (error-exception-message e))
          (let ()
            ...
            (cond
              ...
              ((equal? (error-exception-message e) 'not-a-callable-value)
               (let ()
                 (define value (car (error-exception-parameters e)))
                 (show-runtime-error (last (error-exception-parameters e)))
                 (ra::display-err value)
                 (ra::display-err "\n")
               )
              )
              ((equal? (error-exception-message e) 'required-args-not-passed)
               (let ()
                 (define not-passed (car (error-exception-parameters e)))
                 (show-runtime-error (last (error-exception-parameters e)))
                 (ra::display-err
                   (apply string-append
              ...
              ((equal? (error-exception-message e) 'default-passed-from-outside)
               (show-runtime-error (last (error-exception-parameters e)))
               (ra::display-err (car (error-exception-parameters e)))
               (ra::display-err "\n")
              )
              ((equal? (error-exception-message e) 'unexpected-positional-arg)
               (show-runtime-error (last (error-exception-parameters e)))
               (ra::display-err (car (error-exception-parameters e)))
               (ra::display-err "\n")
              )
              ((equal? (error-exception-message e) 'unbound-variable)
               (let ((params (error-exception-parameters e)))
                 (if (null? params)
                   (ra::display-err "Unknown variable\n")
                   (if (> (length params) 1)
                     (show-runtime-error (last params))
                     (begin
                       (ra::display-err "Variable: ")
                       (ra::display-err (car params))
                       (ra::display-err "\n")))))
              )
              ...
              ((member (error-exception-message e)
               ...
               (let ()
                 (define arg (car (error-exception-parameters e)))
                 (show-runtime-error (last (error-exception-parameters e)))
                 (ra::display-err (car arg))
                 (ra::display-err " = ")
                 (ra::display-err (cadr arg))
                 (ra::display-err "\n")
               )
              )
              ((member (error-exception-message e)
